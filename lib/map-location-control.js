 ...
 */
export default function locationControl(options = {}) {
  let map;
  let el;
  const geo = geolocation(options.geolocation);
  const compass = kompas(options.kompas);
  let position = {};
  let tracking = false;
  ...
  function display() {
    if (!position.ll) {
      ...
      const bearing = position.speed > SPEED_THRESHOLD ? position.heading : position.compassHeading;
      if (typeof bearing === 'number') {
        const rotation = bearing - 45; // arrow already rotated 45 degrees
